 ...
 *
 * Supports s3-user-files tools (read, write, list, delete) via child_process.
 */
...
/**
 * Execute a tool by running the corresponding s3-user-files script.
...
function executeTool(toolName, args, userId) {
  ...
  switch (toolName) {
    case "read_user_file":
      scriptArgs = [script, userId, args.filename || ""];
      break;
    case "list_user_files":
      scriptArgs = [script, userId];
      break;
